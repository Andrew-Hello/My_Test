...
def analyze_pdf(path: Path):
    ...
    largest_images = sorted(unique.values(), key=lambda x: (x['compressed_stream_bytes'] or 0), reverse=True)[:20]
    lowest_ppi = sorted(
...
def build_markdown(report):
    ...
    pdfs = report['pdfs']
    lines = ['# PDF Quality Comparison', '', f"Generated: {report['generated_utc']}", '']
    if src:
        lines += [
            '## Source DOCX', '',
