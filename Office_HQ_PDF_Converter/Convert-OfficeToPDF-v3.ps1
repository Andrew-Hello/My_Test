...
$RebuilderScript = Join-Path $ScriptRoot 'Rebuild-HighRes-PDF-Images.py'
...
$Script:LogPath = Join-Path $LogsDirectory ("{0}_Office_HQ_PDF_v3.log" -f $SessionStamp)
...
function Get-TempPdfPath([string]$SourcePath) {
    $root = Join-Path $env:TEMP 'Office_HQ_PDF_Converter'
    New-Item -ItemType Directory -Path $root -Force | Out-Null
    $base = [IO.Path]::GetFileNameWithoutExtension($SourcePath)
    return (Join-Path $root ("{0}_{1}.pdf" -f $base, [guid]::NewGuid().ToString('N')))
}
...
function Ensure-HqPythonDependencies($Runner) {
    $check = Invoke-Python $Runner @('-c', 'import fitz, PIL, imagehash')
    if ($check -eq 0) { return $true }

    Write-Log 'WARN' 'HQ Python modules are missing. Attempting one-time pip installation: pymupdf pillow ImageHash.'
    $install = Invoke-Python $Runner @('-m', 'pip', 'install', '--user', '--upgrade', 'pymupdf', 'pillow', 'ImageHash')
    if ($install -ne 0) {
        Write-Log 'ERROR' 'Automatic Python dependency installation failed.'
        return $false
    }
    $check2 = Invoke-Python $Runner @('-c', 'import fitz, PIL, imagehash')
    return ($check2 -eq 0)
}
...
function Export-WordNative([string]$SourcePath, [string]$PdfPath) {
    ...
    try {
        ...
        Write-Log 'INFO' 'Exporting structural PDF with classic Word ExportAsFixedFormat. Image quality will be repaired afterwards.'
...
function Export-ExcelNative([string]$SourcePath, [string]$PdfPath) {
    ...
    try {
        ...
        Write-Log 'INFO' 'Exporting structural PDF with Excel xlQualityStandard; OOXML image streams will be repaired afterwards.'
...
function Export-PowerPointNative([string]$SourcePath, [string]$PdfPath) {
    ...
    try {
        ...
        Write-Log 'INFO' 'Exporting structural PDF with PowerPoint Print intent; OOXML image streams will be repaired afterwards.'
        try {
...
function Invoke-HqRebuild([string]$SourcePath, [string]$NativePdf, [string]$FinalPdf) {
    if (-not (Test-Path -LiteralPath $RebuilderScript)) {
        Write-Log 'ERROR' 'High-resolution rebuilder script is missing.'
        return $false
    }
    $runner = Find-PythonRunner
    if ($null -eq $runner) {
        Write-Log 'ERROR' 'Python 3 was not found. HQ rebuild cannot run.'
        return $false
    }
    ...
    Write-Log 'INFO' 'Starting high-resolution OOXML image stream rebuild.'
    ...
    if ($exitCode -ne 0) {
        Write-Log 'ERROR' ("HQ image rebuild failed with exit code {0}." -f $exitCode)
        return $false
    }
    ...
    Write-Log 'OK' ("HQ rebuild completed. Diagnostic: {0}" -f [IO.Path]::GetFileName($report))
    return $true
}
...
function Save-SessionDiagnostic([string]$SourcePath, [string]$OutputPath, [string]$Mode, [double]$Seconds, [string]$Message) {
    ...
    $safe = [IO.Path]::GetFileNameWithoutExtension($SourcePath) -replace '[^A-Za-z0-9._-]', '_'
    $path = Join-Path $DiagnosticsDirectory ("{0}_{1}_v3.json" -f (Get-Date -Format 'yyyyMMdd_HHmmss_fff'), $safe)
...
function Convert-One([string]$InputPath) {
    ...
    try {
        ...
        Write-Log 'INFO' ("Structural PDF created: {0:N3} MB" -f ($nativeSize / 1MB))

        if ($ModernExtensions -contains $ext) {
            $final = Get-UniquePdfPath $resolved ''
            if (Invoke-HqRebuild $resolved $nativeTemp $final) {
                ...
                Save-SessionDiagnostic $resolved $final 'HQ_REBUILT' $seconds 'Office layout + lossless OOXML image restoration.'
                return $true
            }
            ...
            Save-SessionDiagnostic $resolved $fallback 'NATIVE_ONLY' $seconds 'HQ rebuild unavailable or failed; output may be ~200 PPI.'
            return $false
        }

        # Legacy binary formats cannot expose OOXML media directly. Preserve a clearly
        # labeled Office-native result rather than pretending it received HQ repair.
        $legacyOut = Get-UniquePdfPath $resolved '_NATIVE_ONLY'
        Move-Item -LiteralPath $nativeTemp -Destination $legacyOut -Force
        $seconds = ((Get-Date) - $started).TotalSeconds
        Write-Log 'WARN' ("NATIVE_ONLY: legacy binary format has no OOXML media package. Created {0}." -f [IO.Path]::GetFileName($legacyOut))
        Save-SessionDiagnostic $resolved $legacyOut 'NATIVE_ONLY' $seconds 'Legacy DOC/XLS/PPT currently uses Office-native export only.'
        return $true
    }
    catch {
        $seconds = ((Get-Date) - $started).TotalSeconds
        Write-Log 'ERROR' ("FAILED: {0}" -f $item.Name) $_.Exception
        Save-SessionDiagnostic $resolved $nativeTemp 'FAILED' $seconds (Get-ExceptionText $_.Exception)
        return $false
    }
    finally {
...
Write-Log 'INFO' 'Office HQ PDF Converter v3 started.'
Write-Log 'INFO' 'Stable strategy: Office layout engine -> native structural PDF -> lossless OOXML image restoration.'
Write-Log 'INFO' 'PDFMaker and ExportAsFixedFormat2 are intentionally NOT used in the production path.'
